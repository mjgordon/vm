...
(defmacro defun-level-head (name body-name child-name &optional (maximize-datatypes t))
  `(defun ,name (branch)
...
(defun generate-exp-relational-body (parent branch)
  (let* ((values (token-value branch))
	 (exp-b (second values))
	 (exp-b-result (generate-exp-additive exp-b))
	 (type-a (token-datatype parent))
	 (type-b (token-datatype exp-b)))
    (setf (token-datatype branch) 'int4)
    (concatenate 'string
		 exp-b-result
		 (case (token-type (first values))
...
(defun generate-exp-equality-body (parent branch)
  (let* ((values (token-value branch))
	 (exp-b (second values))
	 (exp-b-result (generate-exp-relational exp-b))
	 (type-a (token-datatype parent))
	 (type-b (token-datatype exp-b)))
    (setf (token-datatype branch) 'int4)
    (concatenate 'string
...
(defun generate-exp-logical-and-body (parent branch)
    (let* ((values (token-value branch))
	 (exp-b (second values))
	 (exp-b-result (generate-exp-equality exp-b)))
	 ;;(type-a (token-datatype parent))
	   ;;(type-b (token-datatype exp-b)))
    (setf (token-datatype branch) 'int4)
    (concatenate 'string
...
(defun generate-exp-body (parent branch)
      (let* ((values (token-value branch))
	 (exp-b (second values))
	 (exp-b-result (generate-exp-logical-and exp-b)))
	     ;;(type-a (token-datatype parent))
	     ;;(type-b (token-datatype exp-b)))
    (setf (token-datatype branch) 'int4)
    (concatenate 'string
