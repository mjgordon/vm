Generating a mapping of final bytecode to assembly original. 
Start with a simple list of sequential ids for each token (including labels, not including comments) in the source
Each expansion in the dictionary also has knowledge of A. how many tokens it consumes, B. How many tokens it outputs
E.G. A simple logic command may turn 1 token (i.e. AND) into N tokens. 
E.G. A CALL or GOTO will consume the next token (the > ref), and output N tokens (LIT > for COND is normal however)
The expansion recursion passes around an input list and a built list as well, adding N copies of the input list to the built list each time. 
This does not account for linking, this is a far future concern

Note : This file is intended to be viewed in org mode with org-indent-mode turned on
...
** v003 Basic demos and assembly analysis [19/31]
...
- [ ] C AS : Export analysis files with correct names
...
- [ ] R AS : More clearly differentiate symbols for local and global references and labels @ > % etc
- [X] R AS : Rename dictionary.lisp to assembler-dictionary.lisp
- [ ] R AS : Rename util-assembly.lisp to assembler-util.lisp
...
- [ ] F VM : Dumb update cycle, call update pixels every $arg operations.
- [X] F PR : Graphing of opcode usage (program vs runtime)
- [ ] F PR : HTML Heatmap of original program 
- [ ] F PR : Automatic unit-testing script
- [ ] F PR : Elementary cellular automata program (e.g. rule 90 Sierpinski)
- [ ] F PR : Sierpinski carpet program
- [ ] C PR : Opcode graphing should separate constant numbers in assembly
- [ ] B VM : ^C Doesn't exit when stuck in infinite loop
- [ ] B AS : Properly handle trailing whitespace at end of lines
- [ ] C PR : Write decent README.md

** vFUTURE
- [ ] T VM : Speed of main opcode branching being switch statement vs function pointer array. (Checked assembly, *may* be doing an internal jump table right now, can't tell. Saving for when I have a more intensive demo program to test with, so differences won't be rounding errors in scale
...
(regexp-opt '("DROP" "DUP" "SWAP" "CONST3_0" "CONST4_0" "CONST3_1" "CONST4_1" "AND" "OR" "NOT" "TRUE" "ZERO2" "EQUAL3" "ADD21" "ADD22" "ADD31" "ADDC3_1" "ADDC4_1" "SUB21" "SUB22" "SUB31" "SUB33" "MULT" "MULT21" "MULT22" "DIV" "DIV21" "DIV22" "DIV31" "MOD" "MOD21" "MOD22" "MOD31" "OUT" "OUT8" "OUT12" "OUT16"))
...
>   : Label reference, assembler replaces with final address of associated @ label
@%  : Local label, the assembler will turn every expanded instance into a unique label 
>%  : Label reference, assembler replaces with final address of associated @% label in that expansion
